...
const crear        = async (req, res, next) => { try { success(res, await service.crear(req.body), 'Barrio creado', 201); } catch (e) { next(e); } };
const actualizar   = async (req, res, next) => { try { success(res, await service.actualizar(Number(req.params.id), req.body), 'Barrio actualizado'); } catch (e) { next(e); } };
